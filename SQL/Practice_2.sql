...
CROSS APPLY (
    ...
    WHERE o.customer_id = c.customer_id
    ORDER BY o.order_date DESC
) AS o; -- This query retrieves each customer along with their most recent order (if any). The CROSS APPLY allows us to get the top 1 order for each customer based on the order date. If a customer has no orders, that customer will not be included in the result set.
